...
public class CopyRpDataService {
    ...
    private final RosePointCopyTrackerRepository trackerRepository;
    ...
    public CopyRpDataService(ConfigurationMap configurationMap, RosePointDataRepository rosePointDataRepository,
                             EntityAdapter adapter, DateConfig dateConfig, RosePointCopyTrackerRepository trackerRepository, InfluxDBClient influxClientSource) {
        this.configurationMap = configurationMap;
        this.rosePointDataRepository = rosePointDataRepository;
    ...
    public void copy(String assetCode, String assetAbbreviation) throws InterruptedException {
        ...
        List<RosePointData> telemetryDataList = new ArrayList<>();
        ...
        Optional<CopyTrackerRP> trackerRecord = trackerRepository.findById(assetAbbreviation);
        ...
        while (selectStopTime.isAfter(fullStartTime)) {
            ...
            if (!telemetryDataList.isEmpty()) {
                rosePointDataRepository.saveAll(telemetryDataList);
                rosePointDataRepository.flush();
            }

            trackerRepository.saveAndFlush(new CopyTrackerRP(assetAbbreviation, processedRecords, new Timestamp( selectStartTime.toInstant().getMillis())));
